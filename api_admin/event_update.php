...
if ($method === 'POST') {
    ...
    $updateStmt = $conn->prepare("UPDATE $dbtable SET title_en = ?, title_ch = ?, img = ?, date = ?, time = ?, host_en = ?, host_ch = ?, venue_en = ?, venue_ch = ?, description_en = ?, description_ch = ?, ticket_price = ?, enable = ? WHERE uuid = ?");
    $updateStmt->bind_param("ssssssssssssss", $title_en, $title_ch, $img, $date, $time, $host_en, $host_ch, $venue_en, $venue_ch, $description_en, $description_ch, $ticket_price, $enable, $uuid);
